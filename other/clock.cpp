...
void CLOCK::begin()
{
		...
		memset(&cutdown_string,0x00,sizeof(cutdown_string));//清除	
		memset(&dt,0x00,sizeof(dt));//清除	
		memset(&alarm_dt,0x00,sizeof(alarm_dt));//清除	
}

//GPS接收数据后执行,将GPS接收的数据更新时钟
void CLOCK::sec_process(u8 *date,u8 *time,u8 timezone_flag,u8 sec_flag)
{
	set_date(date);
	set_time(time);
...
void CLOCK::set(uint8_t year,uint8_t mon,uint8_t date,uint8_t hour,uint8_t min,uint8_t sec)
{
	dt.year	= year;
	dt.month= mon;
	dt.date	= date;
...
void CLOCK::set_date_time(u8 *date)
{
	dt.year		= uint8_t ((date[5]-0x30) + (date[4]-0x30)*10);
	dt.month	= uint8_t ((date[3]-0x30) + (date[2]-0x30)*10);
	dt.date		= uint8_t ((date[1]-0x30) + (date[0]-0x30)*10);
	
	dt.hour		= uint8_t ((date[7]-0x30) + (date[6]-0x30)*10);
	dt.min	= uint8_t ((date[9]-0x30) + (date[8]-0x30)*10);
	dt.sec		= uint8_t ((date[11]-0x30) + (date[10]-0x30)*10);
	get_week(dt.year,dt.month,dt.date,dt.week);
	date_to_str(dt);
	time_to_str(dt);
};
void CLOCK::timezone_adjust(uint8_t timezone)
{
	dt.hour += timezone;
	if(dt.hour >= 24)
	{
...
void CLOCK::add_one_day()
{
	switch(dt.month)
	{
		case 1:mon_1_3_5_7_8_10_12_day_carry();break;
		case 2:mon_2_day_carry();break;
		case 3:mon_1_3_5_7_8_10_12_day_carry();break;
		case 4:mon_4_6_9_11_day_carry();break;
		case 5:mon_1_3_5_7_8_10_12_day_carry();break;
		case 6:mon_4_6_9_11_day_carry();break;
		case 7:mon_1_3_5_7_8_10_12_day_carry();break;
		case 8:mon_1_3_5_7_8_10_12_day_carry();break;
		case 9:mon_4_6_9_11_day_carry();break;
		case 10:mon_1_3_5_7_8_10_12_day_carry();break;
		case 11:mon_4_6_9_11_day_carry();break;
		case 12:mon_1_3_5_7_8_10_12_day_carry();break;
	}
	
	get_week(dt.year,dt.month,dt.date,dt.week);
}
int	CLOCK::dt_changed(DATA_TIME_STRUCT &_dt)
{
	int ret = 0;
...
//使用基姆拉尔森计算公式
//把一月和二月看成上一年的十三月和十四月
void CLOCK::get_week(u16 year,u8 month,u8 date,u8 &_week)
{	
		year += 2000;
    if(month==1||month==2) {
        month+=12;
...
void CLOCK::alarm_set(uint8_t year,uint8_t mon,uint8_t date,uint8_t hour,uint8_t min,uint8_t sec)
{
	 alarm_dt.year	= year;
	 alarm_dt.month	= mon;
	 alarm_dt.date	= date;
...
void CLOCK::alarm_enable(u8 enable)
{

	_alarm_enable = enable;
}
void	CLOCK::update_cutdown()
{
	int i = 0;
	u32 temp;
	u32 diff_sec;
	...
	if(day_in_year(dt) < day_in_year(alarm_dt))
	{
		...
		{
			temp = days_between_2_date(dt,alarm_dt);
			diff_sec = get_diff_sec(dt,alarm_dt);
		}
		else
		{
			temp = days_between_2_date(dt,alarm_dt);
	...
	cutdown_string[9] = 0;
	cutdown_string[i++] = temp/100 			+ 0x30;
	cutdown_string[i++] = (temp%100)/10 + 0x30;
	cutdown_string[i++] = (temp%100)%10 + 0x30;
	cutdown_string[i++] = h/10 + 0x30;
	cutdown_string[i++] = h%10 + 0x30;
	cutdown_string[i++] = m/10 + 0x30;
	cutdown_string[i++] = m%10 + 0x30;
	cutdown_string[i++] = s/10 + 0x30;
	cutdown_string[i++] = s%10 + 0x30;
}

//////////////////////////////////////////////////////////////////////////////
void	CLOCK::swap(u16 *a,u16 *b)
{
	u16 temp;
	temp=*a;
	*a=*b;
...
//根据给定的日期算出他是该年的第几天
//1月1号算第一天
u16		CLOCK::day_in_year(DATA_TIME_STRUCT &_dt)
{
	u16 temp_year;	
	u16 temp_month;	
	u16 temp_day;	
...
u16	CLOCK::days_between_2_date(DATA_TIME_STRUCT &dt_current,DATA_TIME_STRUCT &dt_target)
{
...
u32	CLOCK::sec_in_day(DATA_TIME_STRUCT &_dt)
{
	return _dt.hour*3600 + _dt.min*60 + _dt.sec;
}
u32 CLOCK::get_diff_sec(DATA_TIME_STRUCT &dt1,DATA_TIME_STRUCT &dt2)
{
...
void	CLOCK::sec_to_time(u32 sec_source,u8 &hour,u8 &min,u8 &sec)
{
...
u8 CLOCK::is_leap_year(u16 _year)
{
...
///////以下是日历计算的私有函数/////////////////////////////////////////////
void CLOCK::mon_1_3_5_7_8_10_12_day_carry()
{
	dt.date += 1;
	if(dt.date > 31)
	{
...
void CLOCK::mon_4_6_9_11_day_carry()
{					
	dt.date += 1;
	if(dt.date > 30)
	{
...
void CLOCK::date_to_str(DATA_TIME_STRUCT &_dt)
{
...
void CLOCK::time_to_str(DATA_TIME_STRUCT &_dt)
{
